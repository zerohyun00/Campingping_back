...
export class ChatController {
  ...
  /**
   * @description 특정 채팅방에서 사용자가 읽지 않은 메시지 개수를 조회하는 API
   * @param roomId 조회할 채팅방의 ID
   * @returns 해당 채팅방에서 읽지 않은 메시지 개수
   */
  @UseGuards(JwtAuthGuard)
  @Get('unread/:roomId')
  @ApiOperation({
    ...
    description:
      '특정 채팅방에서 사용자가 읽지 않은 메시지 개수를 조회합니다. (내가 보낸 메시지는 제외됨)',
  })
  @ApiBearerAuth() // JWT 인증 필요
  @ApiParam({
    name: 'roomId',
  ...
  @ApiResponse({
    status: 400,
    description: '잘못된 요청 (예: 채팅방 ID가 숫자가 아님)',
    schema: {
      example: {
        statusCode: 400,
  ...
  async getUnreadMessageCount(@Param('roomId') roomId: number, @Req() req) {
    const userId = req.user.sub;
